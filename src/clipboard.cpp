...
struct ClipboardTool
{
    ...
    bool (
        *is_available)(); // Опциональная проверка среды, nullptr если не нужна
};
                               ...
                               const char *verb)
{
    ...
    sa_new.sa_flags = 0;
    bool sigpipe_guard_installed = (sigaction(SIGPIPE, &sa_new, &sa_old) == 0);
#endif
    ...
    {
        std::cerr << "scat: copy: '" << tool.name << "' wrote " << written
                  << " bytes out of " << total << "\n";
    }
    ...
    if (verbose)
    {
        ...
        {
            std::cerr << "scat: copy: '" << tool.name << "' exited with code "
                      << rc << "\n";
        }
    }
...
CopyGuard::CopyGuard(bool enabled, bool verbose)
    : enabled_(enabled), verbose_(verbose), old_buf_(nullptr)
{
    if (enabled_)
    {
